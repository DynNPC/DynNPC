...
def cut_in_left(sim, ego, npc, forward, right, num_points, n, m, k):
    ...
    要做随机化处理：
    一、假定ego匀速，可能碰撞的区域一定时在P2——P3之间，按照这个最近/远距离可以计算最短/长时间
    二、在最近距离-半个npc车长处，计算最短时间，获取最快速度；在最远距离+半个npc车长处，计算最长时间，获取最慢速度
    三、分为三个区间，小于最小就是避让、大于最大是超过、区间内是交互性
    四、考虑到ego车是可能减速的，所以在小于最小的区间，也是较有可能碰撞的。
    """
    waypoints = []
    ...
    # 计算速度并赋值给所有路径点
    # total_distance：初始直线 + 以直代曲的变道路径长度
